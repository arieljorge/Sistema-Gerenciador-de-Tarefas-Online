...
public class ContribuicaoController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<Void>> cadastrarContribuicao(@Valid @RequestBody ContribuicaoCreateDto contribuicaoCreateDto) {
        this.contribuicaoService.cadastrarContribuicao(contribuicaoCreateDto);
        return ResponseEntity.ok(new ApiResponse<>(true, "contribuição salva com sucesso.", null));
    }

    @PutMapping
    public ResponseEntity<ApiResponse<Void>> editarContribuicao(@Valid @RequestBody ContribuicaoUpdateDto contribuicaoUpdateDto) {
        this.contribuicaoService.atualizarContribuicao(contribuicaoUpdateDto);
        return ResponseEntity.ok(new ApiResponse<>(true, "contribuição salva com sucesso.", null));
    }

    @GetMapping
    public ResponseEntity<ApiResponse<PageResponseDto<ContribuicaoOutDto>>> obterContribuicoes(
            @PageableDefault(sort = "id", direction = Sort.Direction.ASC) Pageable pageable,
            ContribuicaoFilterDto contribuicaoFilterDto
    ) {
