...
import {type Contribuicao, contribuicaoService} from "@services/contribuicao.service.ts";
import RemoveDialog from "@pages/Contribuicao/components/RemoveDialog.tsx";
import CreateForm from "@pages/Contribuicao/components/CreateForm.tsx";
import EditForm from "@pages/Contribuicao/components/EditForm.tsx";
...
export default function Contribuicao() {
    ...
    const loadData = async (plataforma?: string, page?: number, size?: number) => {
        try {
            const response = await contribuicaoService.obterContribuicoes(
            ...
            setContribuicoes(response.data.content);
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const contribuicaoServiceResponse = await contribuicaoService.obterContribuicoes(palataformasData[0], 0, 25);

                setPlataformas(palataformasData);
                setContribuicoes(contribuicaoServiceResponse.data.content);
                setRowCount(contribuicaoServiceResponse.data.totalElements);
            } catch (err) {
                console.error(err);
            }
        })();
    ...
    const handleRemove = async () => {
        if (!contextId) return;
        await contribuicaoService.deletarContribuicao(contextId);
